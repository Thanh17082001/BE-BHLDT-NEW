...
export class DistrictService {
  ...
  async find(districtQuery: QueryDistrictDto, pageOptionsDto: PageOptionsDto): Promise<PageDto<DistrictDto>> {
    ...
    console.log("find all districts - entities 22", entities);
    console.log("find all districts - itemCount", itemCount);
    const districts = transformToDto(DistrictDto, entities) as DistrictDto[];
    console.log('transform', districts);

    return new PageDto(districts, pageMetaDto);
  }
